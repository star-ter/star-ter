    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden my-4">
      ...
      <div className="grid grid-cols-4 divide-x divide-slate-100 border-b border-slate-100">
        ...
        <div className="p-5 text-center">
          <div className="text-sm text-slate-400 mb-1">월 예상 매출</div>
          <div className="text-2xl font-bold text-slate-800">{formatCurrency(revenue)}</div>
        </div>

        {/* 예상 지출 */}
        <div className="p-5 text-center">
          <div className="text-sm text-slate-400 mb-1">예상 지출</div>
          <div className="text-2xl font-bold text-slate-800">{formatCurrency(totalCost)}</div>
        </div>

        {/* 순이익 - 강조 */}
        <div className={`p-5 text-center ${isProfit ? "bg-emerald-50" : "bg-red-50"}`}>
          <div className={`text-sm mb-1 ${isProfit ? "text-emerald-600" : "text-red-500"}`}>순이익</div>
          <div className={`text-2xl font-bold ${isProfit ? "text-emerald-700" : "text-red-600"}`}>
        ...
        {/* 이익률 */}
        <div className="p-5 text-center">
          <div className="text-sm text-slate-400 mb-1">이익률</div>
          <div className="text-2xl font-bold text-slate-800">{profitMargin.toFixed(1)}%</div>
        </div>
      </div>

      {/* 비용 구조 */}
      <div className="p-6">
        <div className="text-sm font-medium text-slate-500 mb-3">비용 구조</div>
        ...
        <div className="flex gap-6 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-slate-700">{rentPercent.toFixed(1)}%</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-sm bg-slate-300"></div>
            <span className="text-slate-500">변동비</span>
            <span className="font-medium text-slate-700">{variablePercent.toFixed(1)}%</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-sm bg-slate-700"></div>
            <span className="text-slate-500">순이익</span>
            <span className="font-medium text-slate-700">{profitPercent.toFixed(1)}%</span>
          </div>
        </div>
      </div>
    </div>
